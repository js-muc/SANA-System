...
export default function Reports() {
  ...
  const filteredReports = useMemo(() =>
    ...
    // Default to "all selected" whenever the visible list of reports changes
  // (new term/year/assessment/mode picked, or the search box filters the list)
  useEffect(() => {
    setSelectedIds(new Set(filteredReports.map(r => r.student.id)));
  }, [filteredReports]);
  ...
  // ── Build printable HTML string for a single report ───────────────────────
    ...
    // Builds just the printable INNER content for one report — no <html>/<head>/<body> wrapper.
  // Used by both the single-report builder and the batch (multi-student) builder below,
  // so the two can never drift out of sync with each other.
  function buildReportBodyHTML(reportId: string): string {
    const report = reports.find(r => r.student.id === reportId);
    if (!report) return '';
    const schoolName = school?.name ?? 'School';
    const className = (report.student as any).class?.name ?? '—';
    const assessmentLabel = reportMode === 'summary' ? 'Term Summary' : selectedAssessment;
  ...
  <h1>${schoolName}</h1>
  <h2>CBC Student Report Card — ${report.term} ${report.year} — ${assessmentLabel}</h2>
</div>
...
<div class="footer-note">Generated by SANA OS · ${new Date(report.generatedAt).toLocaleDateString('en-KE', { day: 'numeric', month: 'long', year: 'numeric' })}</div>`;
  ...
  // Single report → full standalone HTML document (used by download/print of one student)
...
${buildReportBodyHTML(reportId)}
</body></html>`;
  }

  // Multiple reports → ONE standalone HTML document, each student's card on its own printed page
  function buildBatchHTML(reportIds: string[]): string {
    const pages = reportIds
      .map(id => `<div class="report-page">${buildReportBodyHTML(id)}</div>`)
      .join('');

    return `<!DOCTYPE html>
<html><head><meta charset="utf-8"/>
<title>Report Cards — ${selectedTerm} ${selectedYear}</title>
<style>${PRINT_CSS}
...
${pages}
  ...
  function printReport(reportId: string) {
    ...
    // ── Batch download/print for whichever reports are currently checked ──────

  function downloadSelected() {
    if (selectedIds.size === 0) return;
    setBatchBusy(true);

    // Keep the same order the list is displayed in, not the arbitrary Set order
    const orderedIds = filteredReports.map(r => r.student.id).filter(id => selectedIds.has(id));
    const html = buildBatchHTML(orderedIds);

    const element = document.createElement('div');
    element.innerHTML = html;

    const modeLabel = reportMode === 'summary' ? 'Term-Summary' : selectedAssessment;
    const opt = {
      margin: 5,
      filename: `Report-Cards_${selectedTerm}_${selectedYear}_${modeLabel}_${orderedIds.length}-students.pdf`
        .replace(/\s+/g, '-'),
      image: { type: 'jpeg', quality: 0.98 },
      html2canvas: { scale: 2 },
      jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' },
    ...
    (html2pdf as any)().set(opt).from(element).save();
    // html2pdf's .save() Promise resolution varies by version, so don't rely on it
    // alone to clear the busy state — fall back to a timeout as a safety net too.
    setTimeout(() => setBatchBusy(false), 3000);
  }
  ...
  return (
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 mb-5 overflow-x-auto">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-4 py-3 mb-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-slate-700 border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold text-white bg-blue-700 rounded-lg hover:bg-blue-800 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              <Download className="w-4 h-4" /> {batchBusy ? 'Preparing PDF...' : `Download Selected (${selectedIds.size})`}
            </button>
          </div>
        </div>
      )}
          ...
          {filteredReports.map(report => {
            ...
            return (
              <div key={report.student.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                <input
                    type="checkbox"
                    checked={selectedIds.has(report.student.id)}
                    onChange={() => toggleSelected(report.student.id)}
